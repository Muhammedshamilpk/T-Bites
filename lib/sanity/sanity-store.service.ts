...
/** Fetch All Published & Active Restaurants for Customer App */
...
/** 1. Fetch Specific Restaurant Details from Sanity CMS */
export async function getSanityRestaurantDetails(targetRestaurantId?: string): Promise<SanityRestaurantDetails | null> {
  ...
  const query = `*[_type == "restaurant" && (_id == $restaurantId || slug.current == $restaurantId)][0] {
    _id,
    name,
    ownerName,
    email,
...
export async function updateSanityRestaurantProfileAction(data: Partial<SanityRestaurantDetails>, targetRestaurantId?: string) {
  try {
    const restaurantId = targetRestaurantId || await getEffectiveRestaurantIdFromCookies();
    if (!restaurantId) return { success: false, message: "No active store reference" };

    const restaurant = await getSanityRestaurantDetails(restaurantId);
...
/** 2. Fetch Menu Items ISOLATED strictly by restaurant reference */
export async function getSanityMenuItems(targetRestaurantId?: string): Promise<SanityMenuItem[]> {
  const restaurantId = targetRestaurantId || await getEffectiveRestaurantIdFromCookies();
  
  if (!restaurantId) {
    console.warn("getSanityMenuItems called without restaurantId. Returning empty list to prevent cross-tenant leakage.");
    return [];
  }

  // Strict restaurant reference filtering: restaurant._ref == $restaurantId
  const query = `*[_type == "menuItem" && restaurant._ref == $restaurantId] | order(_createdAt desc) {
    _id,
    name,
    price,
    description,
...
/** Add Menu Item to Sanity CMS with strict restaurant reference binding */
export async function addSanityMenuItemAction(formData: FormData, targetRestaurantId?: string) {
  try {
    const restaurantId = targetRestaurantId || await getEffectiveRestaurantIdFromCookies();
    if (!restaurantId) {
      return { success: false, message: "Missing restaurant reference. Cannot add menu item." };
    }
...
/** 3. Fetch Kitchen Orders ISOLATED strictly by restaurant reference */
export async function getSanityOrders(targetRestaurantId?: string): Promise<SanityOrder[]> {
  const restaurantId = targetRestaurantId || await getEffectiveRestaurantIdFromCookies();

  if (!restaurantId) {
    console.warn("getSanityOrders called without restaurantId. Returning empty list to prevent cross-tenant order leakage.");
    return [];
  }

  // Strict restaurant reference filtering: restaurant._ref == $restaurantId
  const query = `*[_type == "order" && restaurant._ref == $restaurantId] | order(orderTime desc) {
    _id,
    "orderId": orderId,
    customerName,
    customerPhone,
...
/** 4. Fetch Categories strictly by restaurant reference */
export async function getSanityCategories(targetRestaurantId?: string): Promise<SanityCategory[]> {
  const restaurantId = targetRestaurantId || await getEffectiveRestaurantIdFromCookies();
  
  const query = restaurantId
    ? `*[_type == "category" && (restaurant._ref == $restaurantId || !defined(restaurant))] | order(displayOrder asc) {
        _id,
        name,
        displayOrder,
        "imageUrl": image.asset->url
...
/** 5. Fetch Dashboard Settings strictly by restaurant reference */
export async function getSanityDashboardSettings(targetRestaurantId?: string): Promise<SanityDashboardSettings> {
  const restaurantId = targetRestaurantId || await getEffectiveRestaurantIdFromCookies();
  
  const query = restaurantId
    ? `*[_type == "dashboardSettings" && restaurant._ref == $restaurantId][0] {
        _id,
        notificationSound,
        notificationEnabled,
        onlineStatus,
